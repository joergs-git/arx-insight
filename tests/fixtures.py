...
def make_set(set_id: int, exercise: int, start: datetime, *, reps: int = 8, sec_per_dir: float = 5.0,
             ...
             hidden: bool = False, finished: bool = True, user: int = 1, protocol: int = 3) -> dict:
    ...
    return {
        ...
        "HIDEFROMSTATS": hidden,
    }


class _Cursor:
    """The three queries arx_report issues, answered from the generated rows."""
    ...
    def execute(self, sql: str, params=()):
        ...
        elif "serializeddetaileddata" in q:                             # load_curve
            r = next(r for r in self._rows if r["ID"] == params[0])
            self._result = [(r["SERIALIZEDDETAILEDDATA"], r["EVENTSTREAMDATA"])]
        elif "select repschemedata" in q:                               # featured_settings
            r = next(r for r in self._rows if r["ID"] == params[0])
            self._result = [(r["REPSCHEMEDATA"],)]
        else:
