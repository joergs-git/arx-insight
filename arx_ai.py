...
def build_payload(report: dict, cfg: dict, previous: list[dict] | None = None) -> dict:
    ...
    # how the texts talk to this person (v0.26.0): the DERIVED rules only - never the raw answers as a label
    rules = report.get("coaching_rules") or {}
    profile["wording_rules"] = {"frame": rules.get("frame", "gain"), "compare": rules.get("compare", "self"), "tone": rules.get("tone", "push")}
    ...
    if pva:
        plan_vs_actual = {"plan_made_on": pva["plan_created"], "planned_for": pva["planned_date"], "trained_on": pva["actual_date"],
                          ...
                          "mode_hints_followed": [pva.get("modes_followed"), pva.get("modes_hinted")],
                          "modes": [{"exercise": r["name"], "hint": r.get("mode_hint"), "done_in": r.get("mode_actual")} for r in pva["exercises"] if r.get("mode_hint")]}
        ...
        planner_block = {
            ...
            "proposal": {"date": ns["date"], "session_type": ns["session_type"], "regions": ns["regions"], "est_minutes": ns["est_minutes"],
                         ...
                         "rows": [{"order": x["order"], "exercise": x["name"], "sets": x["sets"], "target": F(x["target_peak_kg"]) or 0,
                                   ...
                                   # after the FIRST miss the same change is the athlete's alternative to the intent cue (v0.19.1)
                                   "settings_alternative": ({("seconds_per_direction" if k == "tempo_s" else k): v for k, v in x["settings_alternative"].items()}
                                                            if x.get("settings_alternative") else None),
                                   # a missed target far below the athlete's own best: effort, not the protocol (v0.19.1)
                                   "effort_note": _text(x.get("effort_note")),
                                   # a mode suggestion for this row (goal first, then situation; v0.15.0) - the row's target stays
                                   "mode_hint": ({"mode": x["mode_hint"]["mode"], "settings": x["mode_hint"]["settings"], "engine_says": _text(x["mode_hint"]["interp"])}
                                                 if x.get("mode_hint") else None),
                                   # the fourth lever after repeated misses: the machine's Inroad Mode at the calibrated value (v0.16.0)
                                   "inroad_mode_pct": x.get("inroad_mode_pct"), "inroad_mode_note": _text(x.get("lever_note")),
    ...
    for e in report.get("exercises", []):
        ...
        exercises.append({
            ...
            "share_of_best_pct": e.get("share_of_best_pct"),      # last comparable force against the athlete's own best (v0.19.1)
    ...
    cal = report.get("inroad_calibration") or {}
    history = {"training_days": report.get("training_days"),
               # the machine's Inroad Mode setting that reaches the goal's fatigue target, from the athlete's own sets (v0.16.0)
               "inroad_calibration": ({"sets": cal.get("n_sets"), "fit": cal.get("all"), "machine_inroad_for_goal_pct": cal.get("machine_for_goal"),
                                       "per_exercise": {n: {"machine_inroad_for_goal_pct": planner.machine_setting(f, planner.EFFORT_TARGETS[planner.goal_effort(cfg.get("goal") or {})]["inroad_min"]), "n": f["n"], "r2": f["r2"]}
                                                        for n, f in (cal.get("exercises") or {}).items()}} if cal else None), "sets": {"working": report.get("sets_working"), "recorded": report.get("sets_total"),
...
12. A missed effort target is answered, never just noted. history.exercises[].effort_misses_in_a_row counts the exercise's last training days in a row that ended below inroad_target_pct (last_inroad_pct = the latest); a day planned sub-maximal (careful, light, limited) is not a miss. ONE miss: the number holds and the athlete gets TWO ways, once: intent - all-out from the first repetition (a start deficit is given away), building the force fast but smoothly (the machine sets the speed - never a jerk), the set ends when the force breaks down, not when the repetitions are over - OR the set-up change the row already carries as planner.proposal.rows[].settings_alternative (pauses at the turnarounds to 0 s, a strength goal keeps 2 s, one second slower per direction up to 5; the comparison basis restarts). Say the intent line ONCE on the whole board - not in the consequence, the cue and the recommendations all three. TWO misses in a row: the set-up changes and the row's tempo field says so - the engine proposes planner.proposal.rows[].settings_change (seconds per direction +1 up to 5, the pauses at the turnarounds to 0 or 2 s; two repetitions more once those are exhausted): keep it or choose the repetitions instead, but never hold the number a third time without changing something, and say that the comparison basis restarts with new settings. The harder third way exists only for an experienced athlete whose history.exercises[].share_of_best_pct is at least 80: a fatigue-target set (planner.proposal.rows[].mode_hint dynamic/fatigue - on arx-free the protocol ends the set itself, on the original a high repetition count and the athlete ends it when the force no longer comes back). Below 80 the gap is effort, not the protocol (the row says so in effort_note): say it plainly and propose no harder set-up. While last_session.session_open is true the consequence of a missed effort target is: once more, NOW, properly - say it first in last_session.consequence (last_session.repeat_now lists the exercises). Afterwards: a set that did not reach the target caused no rest debt - sets not taken to failure recover in about a day, so those muscles are ready the next day and the plan re-plans the same exercises (planner.proposal.repeat_note); never make the athlete wait for a rhythm after a fruitless session. A BEGINNER (profile.experience "new" with fewer than two training days; planner.proposal.beginner_note is set) is the exception to all of this: the target is moderate on purpose, "about half of what you have", learn the movement, next time "beat your previous number by what feels comfortable" - nothing is a miss, nothing escalates, never all-out from rep 1. For a STRENGTH goal the pauses at the turnarounds stay (rest-pause serves tension): the lever is tempo, then repetitions.
13. A row may carry planner.proposal.rows[].mode_hint - a SUGGESTION keyed on the athlete's goal first, then the situation: static/time (a Countdown hold at a pain-free strong position - the original shows an Inroad zone but never ends a set by itself and judges nothing for a hold, the app judges the fatigue from the curve; on arx-free the hold ends by the fatigue target) for an exercise on "go easy" or the first session after months away; dynamic/time (Countdown, seconds and an Output target) on big exercises when the goal carries a conditioning share; dynamic/reps/negative (one session of negative-only repetitions) as a plateau lever under a strength goal. Adopt it in the row's tempo field or say in why why not; never invent a mode the engine did not suggest, never promise a fatigue judgement for a hold or a single-phase set beyond what the data gives, and always say that a mode change restarts the comparison basis. plan_vs_actual.modes says whether the athlete followed a hint. history.inroad_calibration and a row's inroad_mode_pct are the MACHINE's scale (what its Inroad Mode is set to), derived from the athlete's own sets - quote them only as "Inroad-Modus X %" and never as a fatigue figure, and the athlete ends the set when the zone is missed - never say the machine stops it; without a calibration never invent a machine value. Under a maintain goal the engine walks the Inroad ladder (one step of 5 down while strength holds, up when it drops) - keep its numbers.
14. Adherence and wording (verified evidence, science adherence_and_motivation): adults reach 60-80 % of their planned sessions - judge consistency against that band, never against 100 %, and greet a return after a gap as the thing that counts. Positive first and TRUE (only what the data measured - "above last time", never "your best" unless it is one), gain frame (what a set gains, never what a miss costs), the athlete's own last comparable set as the only reference, a repeat or a harder set as an OFFER the athlete decides, never an order or a put-down.
15. profile.wording_rules is how this athlete asked to be talked to (or what the evidence says for their age band): frame "gain" = say what a set gains ("that builds ..."), frame "keep" = say what a set keeps ("that holds your strength for ..."); compare "self" = the only comparison is the athlete's own last set, "others" = the partner's or a peer's number may be named as a friendly comparison, "none" = no comparisons at all, feeling and progress in the athlete's own words; tone "push" = the energetic coach, "numbers" = lead with the figures and keep the words few, "calm" = short, quiet sentences, no exclamation marks. Never mention these rules or any "type" to the athlete.
