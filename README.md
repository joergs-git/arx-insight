...
- 🔁 **A missed effort target is acted on** (v0.10.0) — the coach facts do not just note that a set ended too early. Once: the number holds and the row offers two ways - intent (all-out from the first repetition, the set ends when the force breaks down, not when the repetitions are over) or the set-up change at once (pauses at the turnarounds to 0 s, a strength goal keeps 2 s; one second slower per direction) *(v0.19.1)*. Twice in a row: the change becomes the plan — one second slower per direction (never beyond 5 s, i.e. inside the tempo band where results are equal), pauses to 0 / 2 s, same repetitions; once those are exhausted, two repetitions more. A **fatigue session** (a set ended by the fatigue target) is offered only to an experienced athlete at 80 % or more of his best - below that the gap is effort, not the protocol. The row shows the change next to the old settings, and the comparison basis restarts with the new settings. A day planned sub-maximal (careful, light, limited) is not a miss.
...
**static hold** of about 40 s at a pain-free, strong position, as a Countdown - the original's Inroad zone judges
nothing for a hold, Insight judges the fatigue from the curve, arx-free ends the hold by the fatigue target (holds
cost less recovery than the eccentric overload); a plateau under a **strength** goal → one session of **negative-only**
...
itself at its start; since v0.23.0 the athlete line also carries the person's language and the display units when ARX
Insight knows them, so a person is edited in one place -, the fatigue rule `effort-v3` and the hold rule `hold-1` with shared test vectors, and the words a person reads
...
number holds and the row offers two ways, your choice *(v0.19.1)*: intent (all-out from the first
repetition, the set ends when the force breaks down) or the set-up change at once. After **two** in a row it changes the parameters: one second
...
**strength** goal a short, defined turnaround of 2 s is kept when the set-up changes — a pause of a few
seconds lets the muscle go slack but restores almost nothing (`science.json` → *rest_between_repetitions*:
phosphocreatine recovers with a half-time of about 21 s; real intra-set rests of 10 s and more are what
makes a set easier) — so the lever is the tempo, then the repetitions *(v0.12.0, v0.19.1)*. The harder
third way, a **fatigue session** — a set that ends by the fatigue target (arx-free ends it itself; on the
original: a high repetition count, and you end the set when the force no longer comes back) — is offered
only to an experienced athlete whose last comparable force is at least 80 % of his best; below that the
row says plainly that the gap is effort, not the protocol. And every intent cue reads the same way ARX
coaches it: build the force fast but smoothly, the machine sets the speed, never a jerk.
