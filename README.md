...
twice for **not for me**, three times for **not possible right now (health)** *(v0.8.7)*; a fourth
tap puts it back into the plan. All three take the exercise out of every plan, every suggestion and
out of what the AI coach may choose from. *Health* is for the case where a body part is not simply
"careful" or "avoid" for everything that touches it — a shoulder problem usually forbids the
overhead pressing, not the row: rule out exactly those exercises, set the body part back to *ok*, and
the *limits respected?* box lists the exercise if it appears in your sets anyway. The difference is what
