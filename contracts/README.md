...
|---|---|---|---|---|
| History of the original software for arx-free | ARX Insight (exporter, and since v2 the route `GET /api/export?since=` on its local listener) | `arx-export-6.md` (v6, 2026-09-24: the line kind `person` for everybody Insight knows from arx-free's file alone - `arx_free_id`, the derived Insight id and the person fields, footer `persons`, header `persons=` - arx-free's request 9; never an `athlete` line for them; v5 listed until adopted, older files retired here) - before that `arx-export-5.md` (v5, 2026-09-24: the goal's `fatigue_target_pct` on the athlete line and the original's current `range_of_motion` lines between athletes and sets, footer `ranges` - arx-free's requests 4 and 5) and `arx-export-4.md` (gzip'd NDJSON, verbatim config / events / samples, imperial; the same lines by file or by route, `since` = only the sets that began after a `started_at`; the athlete line carries `language` / `display_units`, `email` (v3) and the motivational profile block `coaching` (v4, 2026-09-24: compare / tone / drive - arx-free's cue chooser reads it, F11) when Insight knows them - "Insight wins for the person, arx-free for the machine", owner 2026-09-23; v3 stays listed until v4 is adopted, v1 / v2 retired here) | arx-free importer + `sync.py` (asks at start and when a session begins, imports what is new, shows the person's fields read-only; takes `email` into `athletes.email`, `coaching` into its cue tags) | importer tests, footer counts, format name in the header (`arx-export-1` names the unchanged line format) |
| What ARX Insight reads from arx-free's database: tables / columns / meanings (`sets.source` NULL = arx-free's own recording, `'arx-original'` = a copy), the curve format, the three settings original / arx-free / both without duplicates, **who is who: the e-mail address first, the import link second, names never**, since v2 the live coach's notes `sets.coach_json` (schema 5) with what Insight computes from them (`coach_effects`, chapter 3), and since v3 (arx-free's request 9) **an athlete that exists only in arx-free is a person of Insight**: five person columns, a stable id derived from arx-free's athlete id, `/api/users` rows with `source` + `arx_free_ids` through which arx-free resolves ANY athlete | **ARX Insight** (the reader states what it relies on; the tables stay arx-free's) | `arx-free-sets-3.md` (2026-09-24; v2 listed until adopted, v1 retired here) | arx-free keeps the columns and meanings stable (a migration that touches one needs a new version here first) and asks the athlete for the e-mail at session start when none is stored | hash in both manifests (adoption open); Insight's `tests/test_sources.py` against a database in that layout |
...
| The words a person reads or hears for the fatigue rule: "Ermüdung im Satz" / "fatigue in the set", levels tief / mittel / leicht, "Ermüdungsziel" / "fatigue target"; since v2 (2026-09-24) a miss is "a real load - the target sits N % higher" (never "half the stimulus"), the repeat is an offer, and four framing rules (positive first and true, gain frame, own reference, offers not orders) bind both products | **ARX Insight** (report, coach) | `vocabulary-2.md` (v1 stays listed until arx-free adopts v2) | arx-free's live display, verdict box and voice (its `v_half` and every "half the stimulus" cue follow) | hash in both manifests (v1 adopted 2026-09-21; v2 adoption open) |
...
| The shape of `history.progress_factors` arx-free's trend view shows (overall index, per exercise status / change / rate / points / Insight's sentence) | **ARX Insight** (the rule and the numbers) | `insight-progress-1.md` (2026-09-24, arx-free's request 8) | arx-free displays, never rebuilds; falls back to its own count without Insight | `tests/test_contracts.py` asserts the shape on a fixture report; a rename = a new version |
