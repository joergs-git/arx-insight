# Contract `arx-free-sets-2` - what ARX Insight reads from arx-free's database

Version 2 (2026-09-24, ARX Insight v0.27.0) - supersedes `arx-free-sets-1` (2026-09-24). What is new (arx-free's
request 6, motivation plan F9 / I8): the column **`sets.coach_json`** (arx-free schema 5, `PRAGMA user_version` 5) -
the live coach's notes about a set arx-free recorded itself, NULL on imported copies. ARX Insight reads it when it is
there (a file without the column is still read - the column is optional for the layout check) and computes per cue
group what the sentence did to the force (report `coach_effects`, chapter 3). `arx-free-sets-1.md` stays in both
folders until arx-free has adopted this file.
