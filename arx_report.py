...
# =============================================================================
# The AI coach lives in arx_ai.py (v0.5.0): payload v2, structured board, validation, memory, chat.
# The error type stays importable from here (the app and older callers use core.AIError).
# =============================================================================
from arx_ai import AIError, classify_ai_error     # noqa: E402,F401  (re-export)
import arx_ai as coach_ai                         # noqa: E402
...
def main():
    ...
    ap.add_argument("--ai", action="store_true", help="Also ask the AI coach for its board (needs an API key)")
    ap.add_argument("--ai-payload", metavar="FILE", help="Write exactly what the AI coach would receive to FILE (no network)")
    args = ap.parse_args()
    ...
    try:
        ...
        if args.ai:
            try:                                  # a failed AI call must never cost the report
                report["ai_board"] = coach_ai.make_board(report, cfg) if coach_ai.has_key(cfg) else None
            except AIError as err:
                report["ai_board"], report["ai_error"] = None, err.info()
                print(f"AI coach failed ({err.code}): {err.message}", file=sys.stderr)
        if args.ai_payload:                       # exactly what WOULD be sent - for a privacy review, no network
            with open(args.ai_payload, "w", encoding="utf-8") as f:
                json.dump(coach_ai.build_payload(report, cfg), f, ensure_ascii=False, indent=2)
            print(f"Wrote the AI payload to {args.ai_payload} (nothing was sent).")
    finally:
        con.close()
